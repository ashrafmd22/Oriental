...
export const BrandCard: React.FC<BrandCardProps> = ({ name, imagePath, category }) => {
  return (
    <div className="group flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-3 transition-all duration-300 hover:-translate-y-1 hover:border-indigo-200 hover:shadow-lg">
      <div className="flex h-24 sm:h-28 items-center justify-center rounded-xl bg-slate-50/70 p-3">
        {imagePath ? (
          <img
            loading="lazy"
            decoding="async"
            src={imagePath}
            alt={`${name} logo`}
            className="h-full max-h-20 w-auto object-contain mix-blend-multiply transition-transform duration-300 group-hover:scale-105"
          />
        ) : (
